...
fn main() {
    ...
    println!("compute units       : {}", a100.compute_units().map_or(0, |v| v.get()));
    println!("warp width          : {}", a100.warp_width().map_or(0, |v| v.get()));
    println!("max threads/unit    : {}", a100.max_threads_per_unit().map_or(0, |v| v.get()));
    println!("registers/unit      : {}", a100.registers_per_unit().map_or(0, |v| v.get()));
    println!("shared mem/unit     : {} KiB",
        a100.shared_mem_per_unit_bytes().map_or(0, |v| v.get()) / 1024);
    println!("L2 cache            : {} MiB",
    ...
    let max_warps = a100.max_resident_warps().expect("all three capacities reported");
    ...
    println!("\nwgpu device: max_resident_warps = {:?} (expected None)", wgpu_topo.max_resident_warps());
}
